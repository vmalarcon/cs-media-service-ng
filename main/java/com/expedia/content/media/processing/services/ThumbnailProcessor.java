...
public class ThumbnailProcessor {
	 ...
	 *
	 * @param url
	 *            Input URL for the image (HTTP and s3 supported).
	 * @param guid
	 *            GUID to use for the image.
	 * @param domain
	 *            Domain the image image belongs to.
	 * @param domainId
	 *            DomainId for the image.
	 ...
	 *
	 * @param fileUrl
	 *            the url of the sent file
	 * @param width
	 *            the width of the returned thumbnail
	 * @param height
	 *            the height of the returned thumbnail
	 * @param rotation
	 *            the rotation of the returned thumbnail
	 * @param guid
	 *            the guid of the message
	 * @param domain
	 *            domain of an imageMessage
	 * @param domainId
	 *            domainId of an imageMessage
	...
	private Thumbnail createGenericThumbnail(final String fileUrl, final int width, final int height,
		...
		try (TemporaryWorkFolder workFolder = new TemporaryWorkFolder(workPath)) {
			sourcePath = FileNameUtil.getSourcePath(fileUrl, guid, workFolder, resourceLoader);
	 ...
	 * 
	 * @param sourcePath
	 *            Locally saved image to convert to thumbnail.
	 * @param width
	 * @param height
	 * @param rotation
	 * @return Path to the thumbnail stored locally.
	 * @param addThumbnailExtension
	 *            boolean value to determine wheter to a a _t extension to the
	 *            file
	 * @throws IM4JavaException
	 *             Thrown when the thumbnail generation fails.
	 * @throws InterruptedException
	 *             Thrown if the convert command fails.
	 * @throws IOException
	 *             Thrown if the convert command fails to read or write.
	 */
	 ...
	 * @throws IM4JavaException
	 *             thrown when any error/warning is returned form the command
	 *             execution
	 */
	 ...
	 * 
	 * @param thumbnailPath
	 *            The thumbnail work file.
	 * @param guid
	 *            The assigned media guid to the media file.
	 * @param domain
	 *            The domain the media belongs to.
	 * @param domainId
	 *            The id, in the domain, of the item the media belongs to.

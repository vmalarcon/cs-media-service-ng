...
public class MediaDBDomainCategoriesDao implements DomainCategoriesDao {
    ...
    private List<Category> getAllMediaCategoriesWithSubCategories(String domain) {
        ...
        jdbcTemplate.query((Connection connection) -> {
            PreparedStatement statement = connection.prepareStatement(ALL_CATEGORIES_AND_SUBCATEGORIES_QUERY);
    ...
    private List<Category> getMediaCategoriesWithSubCategoriesByLocaleId(String domain, String queryLocaleId) {
        final Map<String, Category> categoryMap = new HashMap<>();
        jdbcTemplate.query((Connection connection) -> {
            PreparedStatement statement = connection.prepareStatement(CATEGORIES_AND_SUBCATEGORIES_BY_LOCAL_ID_QUERY);
            statement.setString(1, domain);
    ...
    private void addDomainCategoryToMap(DomainCategory domainCategory, Map<String, Category> categoryMap) {
        if (domainCategory.getParentCategoryId() == null) {
            Category category = categoryMap.get(domainCategory.getCategoryId());
            if (category == null) {
                LocalizedName localizedName = new LocalizedName(domainCategory.getLocalizedName(), domainCategory.getLocaleId());
            ...
            Category category = categoryMap.get(domainCategory.getParentCategoryId());
            if (category == null) {
                LocalizedName localizedName = new LocalizedName(domainCategory.getLocalizedName(), domainCategory.getLocaleId());
                Subcategory subcategory = new Subcategory(domainCategory.getCategoryId(), new ArrayList<>(Arrays.asList(localizedName)));
